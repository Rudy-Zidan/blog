...
class CommentReactionTest < ActiveSupport::TestCase
  test "association must exist" do
    comment_reaction = CommentReactions::LikeReaction.new
    comment_reaction.valid?

    assert_equal(false, comment_reaction.valid?)
    assert_equal("User must exist", comment_reaction.errors.full_messages[0])
    assert_equal("Comment must exist", comment_reaction.errors.full_messages[1])
  end
